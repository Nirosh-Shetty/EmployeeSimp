...
router.put(
  ...
  async (req, res) => {
    try {
      await employeeValidationSchema.validate(req.body, { abortEarly: false });
      const { name, email, mobile, designation, gender, courses, oldImage } =
        req.body;
  ...
  try {
    ...
    if (existingCourses) {
      ...
      const newCourses = courses.filter(
        (course) => !existingCourses.courses.includes(course)
      );
      ...
      if (newCourses.length > 0) {
        ...
        res
          .status(200)
          .json({ message: "Courses added successfully", existingCourses });
      } else {
        res.status(201).json({
          message: "No new courses to add. All courses already exist.",
          existingCourses,
        });
      }
    } else {
      ...
      res
        .status(200)
        .json({ message: "Courses created successfully", newCourse });
    }
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.delete("/v1/courses/:courseName", authenticateJwt, async (req, res) => {
  try {
    ...
    if (existingCourses) {
      ...
      res
        .status(200)
        .json({ message: "Course deleted successfully", existingCourses });
    } else {
  ...
  try {
    ...
    if (existingCourses) {
      ...
      if (courseIndex !== -1) {
        ...
        res
          .status(200)
          .json({ message: "Course updated successfully", existingCourses });
      } else {
